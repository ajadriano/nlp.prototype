...
public class DataExactCardinalityFunction implements OWLExpression {
    ...
    public Result<?> execute(OWLDataFactory factory, OWLReasoner reasoner, Object... args) {   
        if (args.length == 2) {
            return new ClassExpressionResult(factory.getOWLDataExactCardinality((Integer)args[0], (OWLDataPropertyExpression)args[1]));
        }
        else if (args.length == 3) {
            return new ClassExpressionResult(factory.getOWLDataExactCardinality((Integer)args[0], (OWLDataPropertyExpression)args[1], (OWL2Datatype)args[2]));
        }        
