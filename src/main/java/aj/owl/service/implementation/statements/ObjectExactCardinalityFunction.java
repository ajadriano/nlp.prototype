...
public class ObjectExactCardinalityFunction implements OWLExpression{
    ...
    public Result<?> execute(OWLDataFactory factory, OWLReasoner reasoner, Object... args) {   
        if (args.length == 2) {
            return new ClassResult(factory.getOWLObjectExactCardinality((Integer)args[0], (OWLObjectPropertyExpression)args[1]));
        }
        else if (args.length == 3) {
            return new ClassResult(factory.getOWLObjectExactCardinality((Integer)args[0], (OWLObjectPropertyExpression)args[1], (OWLClassExpression)args[2]));
        }        
